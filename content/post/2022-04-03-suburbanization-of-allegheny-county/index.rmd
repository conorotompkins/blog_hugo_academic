...
Since the data comes in a wide format, I pivot it long and fix up the new year column to make it easy to graph with.
```{r}
...
The number of housing units in the county stagnated after 1980, which is expected given the collapse of the steel industry.
```{r}
ac_housing_hu %>% 
  st_drop_geometry() %>% 
...
The decennial difference in "gross" housing units shows that growth stagnated from 1960-2010.
```{r}
ac_housing_hu %>% 
  st_drop_geometry() %>% 
  mutate(year = as.character(year) %>% fct_inorder()) %>% 
  ...
  labs(title = "Growth stagnated from 1960-2010",
       x = "Year",
...
This map shows the areas that gained or lost the most housing units from 1940-2019. Dense housing around industrial areas along the Allegheny and Monongahela Rivers was erased. Homestead and Braddock stand out.
```{r}
...
The North Side and the Hill were targets of ["urban renewal"](https://www.bloomberg.com/news/features/2021-06-24/can-a-demolished-black-neighborhood-build-back-better) in the middle of the century. Dense housing in heavily African-American areas were demolished to make way for an [opera house](https://en.wikipedia.org/wiki/Civic_Arena_(Pittsburgh)), the 279 and 579 highways, and parking lots. The highways are directly related to the white flight exodus to the suburbs, especially in the west and north. Those highways made it easy for the new suburbanites to commute longer distances in single passenger vehicles.
...
slope_graph_anim <- animate(slope_graph_anim, fps = 20)
...
A major trend from 1940-2019 is the significant shift in housing from around the core to outlying suburbs.
```{r}
...
scatter_plot_anim <- ac_density %>% 
  ...
  transition_states(year) +
  labs(title = "More housing built in outlying areas over time",
...
scatter_plot_anim <- animate(scatter_plot_anim, fps = 20)
...
The HHUUD10 also contains estimates for the percentage of land in a tract that is "developed" for the years 1992, 2001, and 2011. "Developed" in this context means "covered by an urban land use".
```{r}
ac_dev <- ac_housing %>% 
...
ac_dev %>% 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.ticks = element_blank())
```
I find it interesting that the South Hills area shows more development than the North Hills. I would have expected more development in the North Hills due to the McKnight Road area and Wexford. My guess is that the tracts in the North Hills cover more land area, which drives down the % that is developed. Conversely, the tracts in the South Hills cover less land area, and less of the South Hills is useful for housing construction because of steep hills.

## Leafet map

Insert leaflet map of 1940-2019 change here