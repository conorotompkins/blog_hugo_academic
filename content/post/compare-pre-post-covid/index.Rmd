...
This code pivots the data long and plots the true number of rides per day and the estimate of the underlying trend per model.
```{r fig.height=10}
components(dcmp) %>% 
  ...
  mutate(name = factor(name, levels = c("number_of_rides", "season_adjust",
                                        ...
                                        "random", "remainder"))) %>% 
  filter(!is.na(value)) %>% 
  filter(name == "trend" | name == "number_of_rides" | name == "remainder") %>% 
...
I am not a time series expert, but it appears that the most basic STL model does the best job of identifying the underlying trend in the data.
```{r fig.height=10}
components(dcmp) %>% 
  pivot_longer(cols = number_of_rides:season_adjust) %>% 
  mutate(name = factor(name, levels = c("number_of_rides", "season_adjust",
  ...
  ggplot(aes(time, value, color = .model)) +
  geom_point(alpha = 1, size = .1, color = "#619CFF") +
  annotate(geom = "rect", 
...
Focusing on that model, it appears that the trend dropped in mid-March, but rebounded to normal levels quickly.