...
#content/post/2023-10-31-forecasting-potholes-with-exogenous-variables/
...
I use `new_data` to generate 12 new future observations for each CV `.id` and make a forecast for each `.id`.
```{r}
...
This code simulates high and low scenarios of precipitation. I use these to create scenario forecasts based on varying levels of future precipitation and the temperature data. Then I forecast each scenario with the `arima_exo_lag3` model.
```{r}
...
The model predicts that the scenario with more precipitation will have ~700 more pothole reports in the next year than the scenario with less precipitation.
```{r}
scenerio_fc |> 
  as_tibble() |> 
  group_by(.scenario) |> 
